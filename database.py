# database.py - ПОЛНАЯ ВЕРСИЯ С ПОЛЕМ "СОСТАВ"
...
# Конфигурация пути к базе данных (работает и локально, и на PythonAnywhere)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_NAME = 'vip_sneaker.db'
DATABASE_URL = f'sqlite:///{os.path.join(BASE_DIR, DB_NAME)}'
logger.info(f"Полный путь к базе данных: {DATABASE_URL}")
...
def add_test_products(session):
    ...
    test_products = [
        Product(
            ...
            composition="Верх: кожа. Подкладка: текстиль. Подошва: резина."  # Пример состава для обуви
        ),
        Product(
            ...
            composition="Верх: Primeknit. Подошва: Boost."  # Пример состава для обуви
        ),
        Product(
            ...
            composition="100% хлопок"  # Пример состава для одежды
        )
    ]
...
# Блок для возможности запустить этот файл напрямую для создания/пересоздания БД
if __name__ == "__main__":
    logger.info("--- Запуск ручной инициализации базы данных ---")
    # Опционально: удаляем старый файл БД для чистого старта
    if os.path.exists(DB_NAME):
        logger.warning(f"Найден существующий файл БД ({DB_NAME}). Удаляем для пересоздания...")
        os.remove(DB_NAME)
    init_db()